      ...
      <div class="form-group">
        ...
        <div class="image-preview" v-if="existingImages.length > 0">
          <div v-for="(img, index) in existingImages" :key="'existing-' + index" class="preview-item">
            ...
            <button type="button" @click="removeExistingImage(index)" class="remove-btn">×</button>
          </div>
        </div>
        <p v-else class="hint">暂无图片</p>
      </div>

      <div class="form-group">
        <label>添加新图片（最多9张）</label>
        <div class="image-upload">
          <input type="file" id="images" multiple @change="handleImageUpload" accept="image/*">
          <p class="hint">支持 JPG、PNG 格式，每张图片大小不超过 5MB</p>
        </div>
        <div class="image-preview" v-if="newImagePreviews.length > 0">
          <div v-for="(preview, index) in newImagePreviews" :key="'new-' + index" class="preview-item">
...
const handleImageUpload = (event) => {
  ...
  const totalImages = existingImages.value.length - removedImages.value.length + newImages.value.length
  if (totalImages + files.length > 9) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.put(`/api/items/${route.params.id}`, formData, {
      headers: {
